...
updated: 2026-09-16T18:45:13.554084Z
type: memo
...
### Install Helm chart
```
helm upgrade --install compass oci://ghcr.io/steve-vine/compass/charts/compass --version <X.Y.Z> \
      -n compass -f staging-aws.yaml --set image.tag=<X.Y.Z> --set frontend.image.tag=<X.Y.Z> --wait
```

### Create Admin account
```
kubectl -n compass exec deploy/compass-api -- \
      python -m compass_api.cli create-admin --email you@example.com --password '<|Password|>' --name 'Your Name'
```
