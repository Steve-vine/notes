...
updated: 2026-09-26T09:36:59.743106Z
type: task
...
number: 759
---
**Carries ADR 0077** (Compass shows the state of its own background work), adapted from ISE ADR 0091.
...
**Behaviour.**
- A new **System status** screen, at the end of the sidebar's **Overview** section, visible to **every signed-in Compass user**. Portal-only users don't get it.
- **Running now:** each job in progress, by its plain name ("Reading shared mailboxes", "Syncing the directory", "Rendering a PDF") and how long it has been running. No arguments, people, companies or tenant content, so the screen is safe to show everyone.
...
- New `GET /api/v1/system/status`, readable by any authenticated internal user. The response carries its verdict thresholds so the screen and any future warnings can't disagree.

**Not in this task:** the scheduled-job table (next task) and trend charts, the separate recorder and expiring ticks (the task after).