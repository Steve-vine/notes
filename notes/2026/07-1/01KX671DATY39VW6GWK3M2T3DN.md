...
updated: 2026-07-31T09:05:12.183627Z
type: project
title: ISE
identifier: ISE
next_task_number: 397
start: 2026-07-10
due: 2026-08-31
sprints:
- id: sh9ng2k
...
- id: s39ax46
  ...
  description: |-
    ...
    READ RELEASED to main 2026-07-31 (PRs #356-#360, main 00a5cf0), all 5 Done; staging reset; account-owned-token verify fallback fixed in review (ADR 0062 §2); live smoke completed by Steve 2026-07-31, no defects.

    WRITE (planned with Steve 2026-07-31): second write-capable account token on the Grant-write flow (ADR 0060/0061 pattern); catalogue = update_dns_record T2 (existing records only — no create/delete), set_ip_access_rule T2, set_security_level T2 (incl. Under Attack), purge_cache_urls T1, purge_cache_everything T2, set_pool_enabled T2; NO freeform WAF custom-rule editing (restricted primitives only), NO tunnel actions (no server-side primitive). Cloudflare writes are synchronous — no LRO helper. UI free via ActionsPanel. ADR 0065 (0063/0064 claimed by the parallel EntraID sprint). Tasks ISE-394 (foundation+ADR) → {ISE-395 DNS+cache, ISE-396 security+LB}.
- id: setdxf2
...
- id: sfv5yw0
  ...
  description: This sprint is a collection of small bugs, issues and improvements
assignee: steve
priority: medium
