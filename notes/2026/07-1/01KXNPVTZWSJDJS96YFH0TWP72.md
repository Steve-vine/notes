...
updated: 2026-07-16T18:45:53.800842Z
type: task
title: Issue Loop
assignee: steve
priority: medium
...
**Propose** - Existing Propose remediation button, as today, where is can propose an actionable change, this should be resented as an approval, which if the user has permissions to approve, can be done so in the chat.  The user should also be able to discuss this with the AI and refine it if necessary.  In order to get the AI to perform it the user must prompt the AI to propose it again, either via the button 
