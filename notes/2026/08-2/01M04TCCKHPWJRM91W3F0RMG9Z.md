...
updated: 2026-08-16T08:32:01.852925Z
type: task
...
- [ ] **Portal management** (decided 2026-08-16, supersedes the internal-only assumption): any owner (main or additional) can **add and remove co-owners from the portal** — portal-router endpoint gated by the ownership helper, picking from the user directory. The **main owner is not changeable from the portal** (only the co-owner list); directory read must therefore be reachable under portal auth for owners (adjust the COM-215 gating note accordingly).
- [ ] **Portal display**: detail page shows Owner + Additional owners by name; owners see the manage control.
- [ ] Tests: helper truth table, My Vendors includes co-owned vendors, portal contacts write allowed for additional owner / denied for non-owner, portal co-owner add/remove (owner allowed, non-owner 403, main owner immutable via portal), main-owner normalisation.
