...
updated: 2026-08-18T13:07:13.600725Z
type: task
...
* **Users section** (`admin/UsersSection.tsx`): provenance column/badge (Local vs Entra), active/deactivated state from SCIM, `entra_object_id` on detail; for Entra users the role editor becomes a read-only derived-roles display with "via *group*" provenance chips linking to the Mappings panel; "no mapped roles" surfaced as a warning state. Local users keep today's role editing. The **Mappings panel** itself (from COM-248) lands wherever this task decides it reads best — keep it one click from the Users list.
