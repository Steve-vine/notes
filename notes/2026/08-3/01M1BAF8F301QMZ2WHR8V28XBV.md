...
updated: 2026-08-31T10:05:31.595995Z
type: task
...
**Dropped for now (2026-08-31)** — writing a note onto the leaver's account as it is disabled. Graph's user resource has no `description` property, Compass mirrors no such field, and the usual substitute (an on-premises extension attribute) is read-only through Graph for AD-synced accounts. Not worth designing around until we know which field is actually meant and whether it is writable.

## Implementation

- `models/access_request.py`: `scheduled_for`, `delete_after_days: int | None`, `delete_due_at`, and where the account was deleted (on the subject, where per-subject outcomes already live). Partial indexes on the two "due" queries so each sweep pass is cheap.
- New `AccessChangeKind.user_deleted`, hanging off the existing subject outcomes so one failure never poisons the batch.
